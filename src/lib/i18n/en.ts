/**
 * Canonical English dictionary. `Dict = typeof en` drives it.ts / ru.ts, so
 * every key here MUST be provided (with the same value type) by the other
 * languages or tsc fails. Interpolated strings are functions.
 */
export const en = {
  common: {
    appName: 'Luche',
    back: 'Back',
    cancel: 'Cancel',
    delete: 'Delete',
    continue: 'Continue',
    iUnderstand: 'I understand',
  },
  menu: {
    startFullCheck: 'Start full check',
    orSingle: 'Or choose a single test below',
    previousRecordings: 'Previous recordings',
    aboutA11y: 'About and privacy',
  },
  profile: {
    title: 'Your account',
    account: 'Account',
    openProfileA11y: 'Open account and settings',
    syncDescription: 'Uploaded recordings and results sync to every device signed in with this account. Recent videos stay on this device for 3 days, then load from the cloud when opened.',
    logout: 'Log out',
    loggingOut: 'Logging out…',
    logoutTitle: 'Log out?',
    logoutBody: 'All video files will be removed from this device. Uploaded recordings and results will remain in your account.',
    unuploadedTitle: 'Some videos have not uploaded',
    unuploadedBody: (n: number) => `${n} video${n === 1 ? ' has' : 's have'} not reached your account yet. Logging out will permanently delete ${n === 1 ? 'it' : 'them'} from this device.`,
    logoutAndDelete: 'Log out and delete',
    logoutFailedTitle: 'Could not log out',
    logoutFailedBody: 'Luche could not finish removing local files. Please try again.',
  },
  session: {
    progress: (n: number, total: number) => `Test ${n} of ${total}`,
    skip: 'Skip',
  },
  invite: {
    title: 'Connect with your doctor',
    enterButton: 'Enter a doctor’s code',
    enterPrompt: 'Enter the 4-digit code from your doctor.',
    codePlaceholder: '0000',
    yourDoctor: 'your doctor',
    confirmTitle: (name: string) => `Link with ${name}?`,
    confirmBody: 'They’ll be able to see your recordings and results.',
    allow: 'Allow',
    linkedTitle: 'You’re connected',
    linkedBody: (name: string) => `${name} can now see your recordings.`,
    done: 'Done',
    tryAgain: 'Try again',
    errors: {
      notFound: 'That code wasn’t found. Check it and try again.',
      selfInvite: 'That is your own code. You cannot connect to yourself.',
      expired: 'This code has expired. Ask your doctor for a new one.',
      revoked: 'This code is no longer valid.',
      capReached: 'This code has reached its limit. Ask your doctor for a new one.',
      generic: 'Something went wrong. Please try again.',
    },
  },
  generate: {
    title: 'Your sharing code',
    prompt:
      'Show your permanent code and share it with the person whose recordings you want to see. When they enter it, their recordings become visible to you.',
    generate: 'Show my code',
    yourCode: 'Your permanent code',
    instructions:
      'This code never changes. Share it with them; once they approve it, their recordings appear under your account.',
    shareCode: 'Share this code',
    shareMessage: (code: string) =>
      `Share your data with me on Luche! Tap the button in the top right of the main screen, then tap “Enter a doctor’s code” and paste this code: ${code}`,
    copyCode: 'Copy sharing code',
    tapToCopy: 'Tap to copy',
    copied: 'Code copied',
    error: 'Could not load your sharing code right now. Please try again.',
  },
  about: {
    title: 'About',
    disclaimerTitle: 'Medical disclaimer',
    disclaimerBody:
      'Luche is a research and wellness tool. It is not a medical device and does not provide a diagnosis. Results are for informational purposes only — always consult a qualified clinician about your health.',
    versionTitle: 'Version',
    versionValue: '1.0.0 (beta)',
    commitTitle: 'Latest GitHub commit',
    commitUnavailable: 'Commit information unavailable',
    languageTitle: 'Language',
    supportTitle: 'Support',
    diagnosticsBody:
      'Export the last 200 technical events to help diagnose recording, upload, or analysis failures. The file excludes video, keypoints, email, authentication tokens, and signed URLs.',
    exportDiagnostics: 'Export diagnostics',
    diagnosticsShareTitle: 'Share Luche diagnostics',
    diagnosticsFailedTitle: 'Could not export diagnostics',
    diagnosticsFailedBody: 'Please try again.',
  },
  disclaimer: {
    title: 'Before you start',
    body1lead: 'Luche is a research and wellness tool. It is ',
    notMedicalDevice: 'not a medical device',
    body1tail: ' and does not provide a diagnosis.',
    body2:
      'Results are for informational purposes only. Always consult a qualified clinician about your health. Do not use Luche to make medical decisions.',
  },
  record: {
    cameraAccessNeeded: 'Camera access needed',
    cameraAccessBody: (name: string) =>
      `Luche uses the camera to record a short video of the ${name} test for analysis. Videos are recorded without audio.`,
    openSettings: 'Open Settings',
    grantAccess: 'Grant camera access',
    saving: 'Saving…',
    tapToStart: 'Tap to start',
    tapToEnd: 'Tap to end',
    startA11y: 'Start recording',
    endA11y: 'End recording',
    flipCamera: 'Flip camera',
    preparing: 'Preparing camera…',
    cameraFailedTitle: 'Camera unavailable',
    cameraFailedBody: 'Luche could not start the camera. Close this screen and try again.',
    recordingFailedTitle: 'Recording not saved',
    recordingFailedBody: 'No test was uploaded. Check available storage and try again.',
    reviewTitle: 'How does it look?',
    reviewHint: 'Play it back — could you see the movement clearly, in good light?',
    submit: 'Looks good — submit',
    retake: 'Retake',
    // Spoken aloud during capture (patient may not be looking at the screen).
    cueDone: 'Done',
    flashlight: 'Flashlight',
    zoomIn: 'Zoom in',
    zoomOut: 'Zoom out',
  },
  resultsList: {
    title: 'Previous recordings',
    emptyTitle: 'No recordings yet',
    emptyBody: 'Complete a test and it will show up here.',
    trendsTitle: 'Your progress',
    personProgress: (name: string) => `${name}’s progress`,
    me: 'Me',
    yourRecordings: 'Your recordings',
    noSharedPeople: 'When someone enters your code, their recordings will appear here.',
    noSharedRecordings: 'This person has not uploaded any recordings yet.',
    loadingRecordings: 'Loading shared recordings…',
    sharedLoadError: 'Could not load shared recordings. Check your connection and try again.',
    tryAgain: 'Try again',
    unnamedPerson: 'Unnamed person',
    selectPerson: (name: string) => `Show recordings from ${name}`,
    scorePending: 'Processing',
    fog: 'Freezing',
    severity: 'Severity',
    sharedBy: (name: string) => `Shared by ${name}`,
    videoLoadFailed: 'Could not load this recording',
    videoLoadFailedBody:
      'Your access may have changed, or the signed video link could not be created. Try again.',
    sharedNoScoreBody: 'The recording could not be scored. Ask the person to record this test again.',
    recordingDetails: 'Recording details',
    recorded: 'Recorded',
    duration: 'Duration',
    frames: 'Frames',
  },
  result: {
    fallbackTitle: 'Result',
    sharingUnavailableTitle: 'Sharing unavailable',
    sharingUnavailableBody: "This device can't share files.",
    shareDialogTitle: 'Save or share recording',
    couldNotShare: 'Could not share',
    deleteTitle: 'Delete recording?',
    deleteBody: 'This removes the clip from this device and deletes its uploaded video, keypoints, and result from the server.',
    deleteA11y: 'Delete recording',
    deleteFailedTitle: 'Could not delete recording',
    deleteFailedBody: 'Nothing was removed. Check your connection and try again.',
    cloudAnalysis: 'Your result',
    scoreHint: 'Lower is better · not a diagnosis',
    gradeLabel: (label: string) => `Severity · ${label}`,
    samplePill: 'SAMPLE — placeholder result, not real analysis',
    noScoreTitle: 'No score could be produced',
    noScoreBody: 'The recording did not pass the analysis quality check. Please record the test again. Details:',
    analysisFailed: 'Analysis failed. Please try recording again.',
    retry: 'Retry',
    failedRetry: 'Something went wrong. It will keep trying — or tap Retry.',
    permanentFailed: 'This recording can’t be uploaded. Please record it again.',
    uploading: 'Uploading to server…',
    processing: 'Processing on server…',
    saveShare: 'Save / share video',
    shareWithDoctor: 'Share this video with my doctor',
    backToMenu: 'Back to menu',
  },
  status: {
    uploading: 'Uploading…',
    processing: 'Processing…',
    done: 'Done',
    needs_retry: 'No score',
    failed: 'Failed',
  },
  uploadBanner: {
    keepOpen: (n: number) =>
      `Uploading ${n} recording${n === 1 ? '' : 's'}`,
    retrying: 'Upload failed — retrying',
    attempt: (n: number) => `attempt ${n}`,
    failed: (n: number) => `${n} upload${n === 1 ? '' : 's'} failed`,
    retryAll: 'Retry all',
  },
  toast: {
    saved: 'Saved',
  },
  offline: {
    message: "You're offline — recordings will upload later.",
  },
  recordingCard: {
    fallback: 'Recording',
    a11y: (name: string, date: string) => `${name} from ${date}`,
  },
  testRow: {
    startA11y: (name: string) => `Start ${name} test`,
  },
  auth: {
    signInTitle: 'Sign in to Luche',
    emailSubtitle: 'Enter your email — we’ll send a one-time code.',
    codeSubtitle: (email: string) => `Enter the code we sent to ${email}.`,
    emailPlaceholder: 'you@example.com',
    codePlaceholder: '000000',
    sendCodeError: 'Could not send a code to that email.',
    invalidCode: 'That code was not valid.',
    signInIncomplete: 'Sign-in incomplete.',
    signUpIncomplete: 'Sign-up incomplete.',
    sending: 'Sending…',
    sendCode: 'Send code',
    verifying: 'Verifying…',
    verify: 'Verify',
    useDifferentEmail: 'Use a different email',
    orContinueWithEmail: 'or continue with email',
    genericError: 'Could not sign in. Please try again.',
  },
  severity: {
    normal: 'Normal',
    slight: 'Slight',
    mild: 'Mild',
    moderate: 'Moderate',
    severe: 'Severe',
  },
  // Shared copy for the redesigned instruction screen (section labels + the
  // reassurance/CTA line). Per-test specifics live under `tests.<id>`.
  instruction: {
    demoCaption: 'Here’s what it looks like',
    demoSoon: 'Video guide coming soon',
    setupTitle: 'Set up your phone',
    stepsTitle: 'What to do',
    tipsTitle: 'For a good result',
    sidePrompt: 'Which side are you testing?',
    leftSide: 'Left',
    rightSide: 'Right',
    ready: 'I’m ready',
    reassurance: 'There’s no wrong way to do this. You can retake it anytime.',  },
  tests: {
    gait: {
      name: 'Walking',
      descriptor: 'Walk toward the camera',
      title: 'Walk in front of the camera',
      blurb: 'A quick look at how you walk.',
      timeEstimate: 'About 1 minute · do it once',
      setup:
        'Ask another person to hold the phone and point it at your legs and lower body while you walk. A phone propped upright can’t see your legs.',
      steps: [
        'Ask another person to hold the phone and point it at your legs.',
        'Walk about 10 steps away from them.',
        'Turn around and walk back.',
      ],
      goodTip: 'Your legs and feet stay clearly in view, in good light.',
      avoidTip: 'A propped-up phone, or your legs leaving the frame.',
      frameHint: 'Keep the legs and feet inside the frame',
      cueStart: 'Start walking',
    },
    arisingFromChair: {
      name: 'Standing Up from a Chair',
      descriptor: 'Stand up from a chair',
      title: 'Stand up from a chair',
      blurb: 'A quick check of standing up.',
      timeEstimate: 'Under a minute · 3 times',
      setup:
        'Prop your phone about 2–3 steps away so it can see all of you sitting in the chair.',
      steps: [
        'Sit back in a firm chair with your feet flat on the floor.',
        'Ask another person to hold the phone so it sees your whole body.',
        'Cross your arms over your chest and stand up without using your hands.',
      ],
      goodTip: 'Your whole body and the chair stay in view.',
      avoidTip: 'Using your hands, or sitting out of frame.',
      frameHint: 'Fit your whole body and the chair',
      cueStart: 'Stand up',
    },
    fingerTapping: {
      name: 'Finger Tapping',
      descriptor: 'Tap finger and thumb',
      title: 'Tap your fingers',
      blurb: 'A quick check of finger movement.',
      timeEstimate: 'About 30 seconds',
      setup:
        'Hold your phone, or prop it close, so it clearly sees your hand. You can flip to the front camera to watch yourself.',
      steps: [
        'Ask another person to hold the phone and point it at your hand.',
        'Tap your index finger and thumb together.',
        'As big and fast as you can, 10 times.',
      ],
      goodTip: 'Your hand fills the frame, in good light.',
      avoidTip: 'Hand too far away, or out of view.',
      frameHint: 'Your hand should fill the frame',
      cueStart: 'Start tapping',
    },
    handMovements: {
      name: 'Hand Movements',
      descriptor: 'Open and close your hand',
      title: 'Open and close your hand',
      blurb: 'A quick check of opening and closing your hand.',
      timeEstimate: 'About 30 seconds',
      setup: 'Hold the phone, or prop it close, so it clearly sees your hand.',
      steps: [
        'Ask another person to hold the phone and point it at your hand.',
        'Make a tight fist with your palm facing the phone.',
        'Open your hand wide and close it, 10 times, as fast as you can.',
      ],
      goodTip: 'Your hand fills the frame, in good light.',
      avoidTip: 'Hand too far away, or out of view.',
      frameHint: 'Your hand should fill the frame',
      cueStart: 'Open and close your hand',
    },
    pronationSupination: {
      name: 'Hand Turns',
      descriptor: 'Turn your palm up and down',
      title: 'Turn your palm up and down',
      blurb: 'A quick check of turning your hand over.',
      timeEstimate: 'About 30 seconds',
      setup: 'Hold the phone, or prop it close, so it clearly sees your hand.',
      steps: [
        'Ask another person to hold the phone and point it at your hand.',
        'Stretch your arm out in front of you, palm facing down.',
        'Turn your palm up and down, 10 times, as fast as you can.',
      ],
      goodTip: 'Your hand fills the frame, in good light.',
      avoidTip: 'Hand too far away, or out of view.',
      frameHint: 'Your hand should fill the frame',
      cueStart: 'Turn your palm up and down',
    },
    toeTapping: {
      name: 'Toe Tapping',
      descriptor: 'Tap your toes',
      title: 'Tap your toes',
      blurb: 'A quick check of tapping your toes.',
      timeEstimate: 'About 30 seconds',
      setup: 'Sit in a chair and have someone point the phone at your foot.',
      steps: [
        'Sit in a chair with both feet flat on the floor.',
        'Ask another person to hold the phone and point it at your foot.',
        'Keep your heel down and tap your toes 10 times, as big and fast as you can.',
      ],
      goodTip: 'Your foot stays in view, in good light.',
      avoidTip: 'Foot out of the frame, or lifting your heel.',
      frameHint: 'Keep your foot in the frame',
      cueStart: 'Tap your toes',
    },
    legAgility: {
      name: 'Leg Agility',
      descriptor: 'Raise and stomp your foot',
      title: 'Raise and stomp your foot',
      blurb: 'A quick check of your leg movement.',
      timeEstimate: 'About 30 seconds',
      setup: 'Sit in a chair and prop the phone so it can see your leg and foot.',
      steps: [
        'Sit in a chair with both feet flat on the floor.',
        'Ask another person to hold the phone and point it at your leg and foot.',
        'Raise one foot and stomp it down 10 times, as high and fast as you can.',
      ],
      goodTip: 'Your leg and foot stay in view, in good light.',
      avoidTip: 'Leg or foot out of the frame.',
      frameHint: 'Keep your leg and foot in the frame',
      cueStart: 'Raise and stomp your foot',
    },
    restTremor: {
      name: 'Rest Tremor',
      descriptor: 'Sit still and relax',
      title: 'Rest your hands and stay still',
      blurb: 'A quick check for shaking while you rest.',
      timeEstimate: 'About 15 seconds',
      setup: 'Sit down and prop the phone so it can see your hands resting.',
      steps: [
        'Sit in a chair and rest your hands on the arms of the chair.',
        'Ask another person to hold the phone so it sees your hands.',
        'Keep your hands still and relaxed for about 15 seconds.',
      ],
      goodTip: 'Your resting hands stay in view, in good light.',
      avoidTip: 'Moving your hands, or hands out of view.',
      frameHint: 'Keep your resting hands in the frame',
      cueStart: 'Rest your hands and stay still',
    },
  },
};

export type Dict = typeof en;
